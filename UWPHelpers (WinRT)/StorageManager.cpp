...
#if defined(TARGET_IS_16299_OR_LOWER)
...
bool RenameUWP(std::string oldname, std::string newname) {
	...
	if (!state) {
		...
		if (iequals(srcRoot, dstRoot)) {
			...
			UWP_DEBUG_LOG(UWPSMT, " Rename used as move -> call move (%s) to (%s)", poldnameth.c_str(), name.c_str());
...
bool writeFile(const std::string& originalPath, StorageFile file, const std::string& content, bool backup) {
	...
	try {
		...
		std::string errorMessage = "WinRT: ";
		errorMessage += convert(e.message());
		UWP_ERROR_LOG(UWPSMT, errorMessage.c_str());
		state = false;
	}
	return state;
...
bool OpenFile(std::string path) {
	auto uri{ winrt::Windows::Foundation::Uri(convert(path)) };
...
bool OpenFolder(std::string path)
{
	...
	else
	{
		...
		auto wParentString = winrt::hstring(parent.ToWString());
...
bool GetDriveFreeSpace(PathUWP path, int64_t& space)
{
	...
	if (storageItem != nullptr) {
		winrt::hstring freeSpaceKey = winrt::hstring(L"System.FreeSpace");
		winrt::Windows::Foundation::Collections::IVector<winrt::hstring> propertiesToRetrieve{ winrt::single_threaded_vector<winrt::hstring>() };
		propertiesToRetrieve.Append(freeSpaceKey);
		winrt::Windows::Foundation::Collections::IMap<winrt::hstring, winrt::Windows::Foundation::IInspectable> result;
